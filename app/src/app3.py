...
while True :
    ...
    dbClient = InfluxDBClient('192.168.10.3', 8086, 'root', 'root', 'Battery')
    ...
    mydb = mysql.connector.connect(host = "192.168.10.3",port='3306',user="admin",password="root",database="BESS")
    ...
    r = redis.Redis(host='192.168.10.3', port=6379)
    now = datetime.now()
    ...
    #---------------------------------Insert Time---------------------------------#
    mydb = mysql.connector.connect(host = '192.168.10.3',user="admin",password='root',database="pengukuran")
    mycursor = mydb.cursor()
